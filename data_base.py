...
class Connector:
	def __init__(self, rules = None):
		self.rules = rules
	def execute(self, script, fetchall = None):
...
def id_name_db(id_ = False, name = False):
	if id_ != False:
		...
		return db[0][0]
	if name != False:
		db = cursor.execute(f"select p.idpersonagem from personagem as p where p.nome = '{name}'", True)
		print(db)
		print(db[0][0])
		return db[0][0]
...
def val_personagem_existe(token):
	for per in personagens_db():
		print('Analisado: ' + str(per))
		print('Igual: ' + str(token.title()))
		if token.title() in per:
			print('foi')
			return True
...
def return_fetchall(script):
	db = cursor.execute(script, True)
	return db

def script_sql(script):
	cursor.execute(script)
#UPDATE `elixirdatabase`.`personagem` SET `embedIcon` = 'ctx.guild.icon_url' WHERE (`idpersonagem` = '1');
